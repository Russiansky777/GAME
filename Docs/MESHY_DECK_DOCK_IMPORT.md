...
Director playtest correction: the working dock and outer berth are now intended to be reachable on foot. The former east wall is opened only at the dock mouth; smooth hidden floors, outer-edge containment and a crane-base proxy provide access without detailed plank collision. The dock is part of the safe base. The northeast alternate return and southeast expedition exit remain clear. Boat boarding remains unavailable; the low-tide shoreline still hides the boat from deep inside the hub.
...
Dock-access follow-up: Editor build passed (113.02 s), all 12 regression tests passed without warnings/failures/skips, and Windows BuildCookRun passed (176.74 s). Actual CharacterMovement traverses from the hub through the old wall position to the berth and returns grounded. Edge/jump-height sweeps, crane-base collision and carried-shard base safety passed. Two fresh packaged standing-view captures (`Artifacts/DockAccessReview/final/{dockwalk,hook}.png`) were inspected with exit 0 and no runtime/material errors. The hook hangs over the water; the adjacent working platform is now walkable. No boat boarding or crane-operation system was added.
