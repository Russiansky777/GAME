...
2. M1 mission transaction: the jobs board begins the expedition, Mara's logbook reward resolves once, and the optional artifact can be retained or sold. Out-of-range board use is rejected; repeated use does not restart the tide; speaking to Mara first does not accept the job.
...
2. Manually play the full first trip with physical input: accept the expedition from the jobs board, read objective/risk communication, collect the protected logbook, take or decline the shrine artifact, and return to Mara. Confirm a normal first-play duration of 8–12 minutes.
